...
struct DeliveryInputView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSettings) {
            DeliveryListSettingsView(items: $items)
        }
    }
...
private struct DeliveryListSettingsView: View {
    @Binding var items: [InventoryItem]
    ...
    private let unitOptions = ["g", "kg", "ml", "L", "個", "食", "枚", "本", "袋"]
    private let defaultCategories = ["未分類", "乳製品", "卵", "粉類", "調味料", "野菜", "肉類", "魚介", "飲料", "冷凍品", "乾物", "消耗品"]
        ...
        NavigationStack {
            Form {
                Section(editingId == nil ? "新規追加" : "編集") {
                    ...
                    TextField("現在庫（未設定なら空欄）", text: $currentStockText)
                        .keyboardType(.decimalPad)
                    TextField("発注点（未設定なら空欄）", text: $reorderPointText)
                        .keyboardType(.decimalPad)
                ...
                ForEach(groupedItemsForSettings, id: \.category) { group in
                    Section("登録済み納品リスト / \(group.category)") {
                            ...
                            ForEach(group.items) { item in
                                HStack(spacing: 8) {
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(item.name)
                                            .font(.headline)
                                        Text("現在庫: \(formatQty(item.currentStock, unit: item.unit)) / 発注点: \(formatQty(item.reorderPoint, unit: item.unit))")
                                            .font(.caption)
                                            .foregroundStyle(.secondary)
                                    }
                                    Spacer()
